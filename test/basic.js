...
exports.debounceLeading = function (t) {
  var intervals = [10, 50, 100, 200, 300, 350, 530];
  t.expect(intervals.length + 1);
  var i = 0;
  var incr = $.debounce(function () { i++; }, 200, true);
  intervals.forEach(function (interval) {
    setTimeout(function () {
      incr();
      t.equal(i, 1, "i incremented only once after " + interval + "ms");
    }, interval);
  });
  ...
  setTimeout(function () {
    t.equal(i, 1, "and not incremented again (" + wait + "ms)");
    t.done();
  }, wait);
};
